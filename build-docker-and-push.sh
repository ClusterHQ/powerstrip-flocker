...
docker run --privileged --rm -e DOCKER_GITCOMMIT=`git log -1 --format=%%h` -v `pwd`:/go/src/github.com/docker/docker fix_unsupported_protocol_scheme hack/make.sh binary
